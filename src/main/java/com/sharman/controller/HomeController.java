...
public class HomeController {
	...
	public String callMoleCalculator() {
	  ...
	  @RequestMapping("/Number-Of-Moles") public String
	  calculateMoles(@RequestParam(value = "givenMass", required=false) Double
	  givenMassInput,
	  
	  @RequestParam(value = "molarMass", required=false) Double molarMassInput,
	  
	  @RequestParam(value = "moles", required=false) Double molesInput, Model
	  model) {
	  
	  Calculator calculate = new Calculator();
	  
	  if (molesInput == null) { double moles =
	  calculate.calculateMoles(givenMassInput, molarMassInput);
	  model.addAttribute("givenMass", givenMassInput);
	  model.addAttribute("molarMass", molarMassInput); model.addAttribute("moles",
	  moles);
	  
	  }else if (givenMassInput == null) { double givenMass =
	  calculate.calculateGivenMass(molarMassInput, molesInput);
	  model.addAttribute("molarMass", molarMassInput); model.addAttribute("moles",
	  molesInput); model.addAttribute("givenMass", givenMass); }else if
	  (molarMassInput == null) { double molarMass =
	  calculate.calculateMolarMass(givenMassInput, molesInput);
	  model.addAttribute("givenMass", givenMassInput); model.addAttribute("moles",
	  molesInput); model.addAttribute("molarMass", molarMass); }
